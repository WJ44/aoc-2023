...
for brick in sorted(snapshot, key=lambda brick: brick[0][2]):
    ...
    while not any(cube[2] == 1 for cube in brick):
        supported_by_cubes = [
            (cube[0], cube[1], cube[2] - 1) for cube in brick if (cube[0], cube[1], cube[2] - 1) in pile
        ]
...
def find_falling(falling, fallen):
    ...
    for b in falling:
        for c in b:
            if (c[0], c[1], c[2] + 1) in part_of_brick:
                supports.add(part_of_brick[(c[0], c[1], c[2] + 1)])
    for b in supports:
        if not b in falling and not supported_by[b] - falling - fallen:
            new_falling.add(b)
